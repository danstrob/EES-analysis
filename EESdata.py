...
def prep_data(year, data_files, pos_vars, country_var, country_dict, replace_dict):
    """
    ...
    """
    stata = pd.read_stata("data/" + data_files, columns=pos_vars + [country_var])
    ...
    for i, var in enumerate(pos_vars):
        ...
        df.loc[:, var + "se"] = stata.groupby(country_var)[var].apply(
            stats.sem, nan_policy="omit"
        )
        ...
        df.loc[:, var + "_se"] = stata.groupby(country_var)[var].apply(
            stats.sem, nan_policy="omit"
        )
...
def EES_data():
    ...
    replace_dict = {
        1989: {"LEFT": 1.0, "RIGHT": 10.0},
        1994: {"LEFT": 1.0, "RIGHT": 10.0, "DK": None, "NA": None, 99.0: None},
        1999: {"left": 1.0, "right": 10.0, "dk": None, -1.0: None, "na": None},
        2004: {
            "an 11-point scale (0-10) was used in SE": 0.0,
            "left": 1.0,
            "right": 10.0,
            "d/k, n/a": None,
        },
    ...
    for ckey, cval in country_dict[1994].items():
        ...
        for i, var in enumerate(pos_vars[1994]):
            ...
            if varname in ees94.keys():
                ...
                se = pd.Series(
                    stats.sem(ees94[varname], nan_policy="omit"),
                    index=[var + "se"],
                    name=cval,
                )
    ...
    pos_data[1994] = pos_data[1994].rename(country_dict, columns=column_rename_dict)
